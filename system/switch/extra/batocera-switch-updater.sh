...
link_tarR="/userdata/system/switch/appimages/ryujinx1.2.81.tar.gz"
# --------------------------------------------------------
if [ "$N" = "1" ]; then C=""; else C="$E/$N"; fi
if [ -f "$link_ryujinx" ]; then
    if [ -f "$link_ryujinx" ] && [ $(stat -c%s "$link_ryujinx") -gt 2048 ]; then
	      echo -e "${T}RYUJINX   ${T}❯❯   ${T}/$version/ ${GREEN}SUCCESS"
	          ...
	          if [ -f "$link_tarR" ]; then
	            cp /userdata/system/switch/appimages/ryujinx1.2.81.tar.gz /userdata/system/switch/ryujinx-${version}-linux_x64.tar.gz 2>/dev/null;
			    echo -e "${T}RYUJINX   ${RED}FAIL TO DOWNLOAD LAST GREEMDEV USE 1.2.81 BACKUP INSTEAD   ${T}/1.2.81/ ${GREEN}SUCCESS";
		      #else
...
mkdir /userdata/system/switch/appimages 2>/dev/null
cd /userdata/system/switch/appimages
citronE="/userdata/system/switch/appimages/citron0.4.AppImage"
if [ -f "$citronE" ]; then
    cp /userdata/system/switch/appimages/citron0.4.AppImage /userdata/system/switch/citron.AppImage 2>/dev/null;
else 
    wget -q --show-progress --tries=10 --no-check-certificate --no-cache --no-cookies -O "/userdata/system/switch/appimages/citron0.4.AppImage" "https://foclabroc.freeboxos.fr:55973/share/cfLux0ouyMRCzQPh/citron0.4.AppImage"
    cp /userdata/system/switch/appimages/citron0.4.AppImage /userdata/system/switch/citron.AppImage 2>/dev/null; fi
        if [ -f "$citronE" ] && [ $(stat -c%s "$citronE") -gt 2048 ]; then
		   echo -e "${T}CITRON   ${T}❯❯   ${T}/V0.4/ ${GREEN}SUCCESS";
		else
